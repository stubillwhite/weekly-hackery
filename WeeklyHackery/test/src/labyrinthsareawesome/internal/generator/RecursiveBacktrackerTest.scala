...
class RecursiveBacktrackerTest extends FlatSpec with Matchers with MockitoSugar {
  ...
  private val start = room_0_0
  private val end = room_0_2
   ...
   * [ ]
   */
  it should "select the only valid neighbour if all other neighbours are out of bounds" in {
    // Given
    val state = RecursiveBacktracker.State(1, 3, start, end, Set.empty, Set.empty, List(room_0_0), false)
   ...
   * [-][ ][-]
   */
  it should "select the only valid neighbour if all other neighbours have been visited" in {

    // Given
    val state = RecursiveBacktracker.State(3, 3, start, end, Set.empty, allRooms - room_1_2, List(room_1_1, room_1_0), false)
    ...
    // Then
    nextState should be(state.copy(
      passages = state.passages + Passage(room_1_1, room_1_2),
      visited = state.visited + room_1_2,
      path = room_1_2 :: state.path))
  }
}
